# linux-image-5.12.2-micro_kernel | LZMA | ZRAM LZ4_COMPRESS | IP6 OFF config | green-recorder fix record audio wayland .webm
...
linux-libc-dev_5.12.3-1_amd64.deb на по какому то пока мне не известному феномену прибавляет в графике таким образом что изображение получается чуть ли не как на экране с амолед или ипс экранах повлияло ли включение дробного масштабирования не понятно и сбилден по трафарету ранее составленного ядра 5.9.3 раздаваемого с ним дистрибутива на моем канале https://youtu.be/9B-nTJyEZX0 т.е если вы собрались делать билд используйте не linux-libc-dev_5.12.2-4_amd64.deb , а linux-libc-dev_5.12.3-1_amd64.deb , а там посмотрим что это за связь  стандартный linux-libc-dev лучше не использовать есть предположение что операционную систему скоро можно будет заставить на dev инклюдах и не оборачивать их в дальнейшем , а в виде бинарников сделать баш запускалки. Если кто знает что тот самый fffff цвет был изначально у меня , а не у корпорации которые потом типа написали что они молочный белый цвет изобрели и не исвестно кому дали за это премию , по факту если вы пользуетесь моим дистрибутивом то у вас эксклюзив самого лучшего в мире оригинала дистрибутива почти как от фирмы единственное что мешает это вечное желание каких то сил его переобновить что бы скрыть сам факт существования такого мощно оснащенного дистрибутива , над которым пришлось работать одному по скольку общество ядовито и всем не угодишь некоторые хотят мате из него сделать другие кде и так по цепочке потому что тот же самый фортран я получил и его часть у меня по этому приходится все править самому , какой объем работы и микро часть из него можно глянуть наидя у меня на гит тот же dconf. Но я напомню что чем лучше графика тем больше нужно памяти , а лучше видео памяти вообщем для не кривой картинки нужна производительность и тут проба сделать что бы ноутбук (не стационар) не напрягался с 2 Гигабайтами памяти на борту по этому все было тестировалось на самом слабом видео композиторе ваиланде.
